    ...
    def calculate_derivatives_thermal(cls, net, branch_pit, node_pit, idx_lookups, options):
        ...
        if transient:
            if np.all(length) == False:
                ...
                branch_component_pit[:, JAC_DERIV_DT] = - rho * area * cp * v_init  + alpha \
                                                        + rho * area * cp / delta_t
                branch_component_pit[:, JAC_DERIV_DT1] = rho * area * cp * v_init  + 0 * alpha \
                                                         + rho * area * cp / delta_t
                ...
                #     -(rho * area * cp * (t_m - tvor) * (1 / delta_t) + rho * area * cp * v_init * (
                #                 -t_init_i + t_init_i1 - tl) / length
                #       - alpha * (t_amb - t_m) * (2 *np.sqrt(np.pi*area)) + qext)
